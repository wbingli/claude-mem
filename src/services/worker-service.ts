...
import {
  handleCodexCommand
} from './integrations/CodexHooksInstaller.js';
...
async function main() {
  ...
  switch (command) {
    ...
    case 'codex': {
      const codexSubcommand = process.argv[3];
      const codexResult = await handleCodexCommand(codexSubcommand, process.argv.slice(4));
      process.exit(codexResult);
      break;
    }
